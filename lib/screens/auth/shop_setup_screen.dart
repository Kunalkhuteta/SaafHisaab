...
class _ShopSetupScreenState extends ConsumerState<ShopSetupScreen> {
  ...
  void _saveShop(bool isEn) async {
    if (_ownerNameController.text.trim().isEmpty ||
        _shopNameController.text.trim().isEmpty ||
        _cityController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(AppLang.tr(isEn, 'Name, shop name, and city are required', 'नाम, दुकान और शहर ज़रूरी है')),
        ...
        children: [
          Container(
            ...
            child: Column(
              ...
              children: [
                Text(AppLang.tr(isEn, 'Set up your shop', 'अपनी दुकान सेटअप करें'),
                    style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.white)),
                const SizedBox(height: 4),
                Text(AppLang.tr(isEn, 'Do it once, use it forever', 'एक बार करें, हमेशा के लिए'),
          ...
          Container(
            ...
            child: Row(
              children: [
                _progressStep('1', AppLang.tr(isEn, 'Login', 'लॉगिन'), true, true),
                _progressLine(true),
                _progressStep('2', AppLang.tr(isEn, 'Setup', 'सेटअप'), true, false),
                _progressLine(false),
                _progressStep('3', AppLang.tr(isEn, 'Start', 'शुरू'), false, false),
              ],
            ),
          ),
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  _buildField(AppLang.tr(isEn, 'Your Name *', 'आपका नाम *'), AppLang.tr(isEn, 'E.g.: Ramesh Sharma', 'जैसे: रमेश शर्मा'), _ownerNameController),
                  const SizedBox(height: 16),
                  _buildField(AppLang.tr(isEn, 'Shop Name *', 'दुकान का नाम *'), AppLang.tr(isEn, 'E.g.: Sharma General Store', 'जैसे: शर्मा जनरल स्टोर'), _shopNameController),
                  const SizedBox(height: 16),
                  _buildField(AppLang.tr(isEn, 'City *', 'शहर *'), AppLang.tr(isEn, 'E.g.: Kota, Rajasthan', 'जैसे: कोटा, राजस्थान'), _cityController),
                  const SizedBox(height: 16),

                  Text(AppLang.tr(isEn, 'Shop Type *', 'दुकान का प्रकार *'), style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: AppColors.textPrimary)),
                  ...
                  const SizedBox(height: 16),
                  _buildField(AppLang.tr(isEn, 'GST Number (optional)', 'GST नंबर (वैकल्पिक)'), '27AAPFU0939F1ZV', _gstController),
                  const SizedBox(height: 32),
                  SizedBox(
                    width: double.infinity, height: 52,
                    child: ElevatedButton(
                      onPressed: _isLoading ? null : () => _saveShop(isEn),
                      ...
                      child: _isLoading
                          ...
                          : Row(
                              ...
                              children: [
                                Text(AppLang.tr(isEn, 'Start Now', 'शुरू करें'), style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.white)),
  ...
  Widget _buildField(String label, String hint, TextEditingController ctrl) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        TextField(
          controller: ctrl, style: const TextStyle(fontSize: 15, color: AppColors.textPrimary),
        ...
        Container(
          ...
          child: Center(
            child: done && !current
                ...
                : Text(num, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: done ? Colors.white : AppColors.textHint)),
          ),
        ),
        const SizedBox(height: 4),
        Text(label, style: TextStyle(fontSize: 10, color: done ? AppColors.primary : AppColors.textHint)),
      ],
    );
  }
  ...
  Widget _progressLine(bool done) {
    return Expanded(
      child: Container(height: 2, margin: const EdgeInsets.only(bottom: 16, left: 4, right: 4), color: done ? AppColors.primary : AppColors.border),
    );
  }
}
